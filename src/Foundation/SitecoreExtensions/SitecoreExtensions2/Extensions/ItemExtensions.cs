    ...
    public static class ItemExtensions
    {
        public static SiteInfo GetSite(this Item item)
        {
            ...
            foreach (SiteInfo siteInfo in siteInfoList)
            {
                ...
                string dataFolder = siteInfo.Properties[Constants.SiteDefinitions.DataFolderProperty];
                if (!string.IsNullOrEmpty(dataFolder)
                    && (item.Paths.FullPath.StartsWith($"{siteInfo.RootPath}{dataFolder}", StringComparison.InvariantCultureIgnoreCase)
                  || item.Paths.FullPath.Equals($"{siteInfo.RootPath}{dataFolder}", StringComparison.InvariantCultureIgnoreCase)))
                {
                    return siteInfo;
                }
